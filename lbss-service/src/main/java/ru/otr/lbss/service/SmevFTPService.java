...
public class SmevFTPService {
	...
	private void init() {
		log.info("init");
		String ftpDirectory = configService.getString(LbssConfig.ftp_directory);
		log.info("FTP Directory : " + ftpDirectory);
		int ftpPort = configService.getInteger(LbssConfig.ftp_port, 21);
		try {
			...
			String dbName = configService.getString(LbssConfig.ftp_users_database);
			ftpUserManager = new FTPUserManager(ftpDirectory, mongoHost, mongoPort, dbName, DocNames.FTPUser,
			        configService.getString(LbssConfig.ftp_admin_password, "admin"));
			FtpServerFactory serverFactory = new FtpServerFactory();
			serverFactory.setUserManager(ftpUserManager);

			FTPListener ftpListener = new FTPListener(null, ftpPort, configService.getBoolean(LbssConfig.ftp_log_enable, false));
			...
			String users = configService.getString(LbssConfig.ftp_users_list);
			if (users != null) {
				createUsers(users);
			}
		} catch (Exception e) {
